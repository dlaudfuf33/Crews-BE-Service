...
public class MembershipService {
    ...
    @Transactional
    public MembershipResponse accountAuthor(Long memberId, Long agitId, AgitAuthorRequest agitAuthorRequest) {
        checkExceptionUtil.validateLeader(memberId,agitId,AgitRole.LEADER);
        Membership membership = checkExceptionUtil.validateRole(agitAuthorRequest.getRequestMemberId(), agitId, AgitRole.ADVANCED);
        if(agitAuthorRequest.getStatus().equals("approve")){
    ...
    @Transactional
    public MembershipResponse memberAuthor(Long memberId, Long agitId, AgitAuthorRequest agitAuthorRequest) {
        checkExceptionUtil.validateLeader(memberId, agitId, AgitRole.LEADER);
        Membership membership = checkExceptionUtil.validateRole(agitAuthorRequest.getRequestMemberId(), agitId, AgitRole.TEMP);
        if (agitAuthorRequest.getStatus().equals("approve")) {
            membership.setAgitRole(AgitRole.MEMBER);
            membershipRepository.save(membership);
            return MembershipResponse.builder()
