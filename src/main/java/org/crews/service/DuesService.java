...
public class DuesService {
	...
	public GetDuesResponse getDues(Long agitId, Long memberId, Integer year, Integer month) {
		...
		for (TransactionHistoryResponse dto : tranList) {
			Optional<Account> optionalAccount = accountRepository.findByAccountNumber(
				AESUtil.encrypt(dto.getCounterpartyAccountNum()));
			if (optionalAccount.isEmpty()) {
				continue;
			}
			Account account = optionalAccount.get();

			Optional<Membership> optionalMembership = account.getMember()
				.getMemberships()
				.stream()
				.filter(memberShip -> memberShip.getAgit().equals(agit) && !memberShip.getCreatedAt()
			...
			Optional<Dues> optionalDues = duesList.stream()
				.filter(dues -> dues.getMembership().equals(optionalMembership.get()) && dues.getDueDate()
					.equals(dto.getTransactionTime()))
				.findAny();
			if (optionalDues.isEmpty()) {
				Dues buildDues = Dues.builder()
					.commonDues(commonDues)
					.dueDate(dto.getTransactionTime())
					.dueAmount(dto.getTranAmount())
					.membership(optionalMembership.get())
					.isPayed(false)
					.accountNumber(account.getAccountNumber())
					.productName(account.getProductName())
					.agitName(agit.getAgitName())
					.standardDate(DateUtil.generateStandardDate(year, month, dto.getTransactionTime()))
					.build();
		...
		return GetDuesResponse.builder()
			.profileResponses(profileResponses)
			.memberCount(profileResponses.size())
			.build();
	}
	...
	public DuesSaveResponse duesSaveCommon(Long agitId, Long memberId, DuesSaveRequest duesSaveRequest) {
		...
		if (optionalCommonDues.isEmpty()) {
			CommonDues buildCommonDues = CommonDues.builder()
				.dueAmount(duesSaveRequest.getDueAmount())
				.dueDay(duesSaveRequest.getDueDay())
				.agit(agit)
				.build();
		...
		return DuesSaveResponse.builder()
			.dueDay(savedCommonDues.getDueDay())
			.dueAmount(savedCommonDues.getDueAmount())
			.build();
	}
	...
	public DuesSaveResponse getDuesCommon(Long agitId, Long memberId) {
		...
		if (commonDues == null)
			...
			return DuesSaveResponse.builder()
				.dueDay(commonDues.getDueDay())
				.dueAmount(commonDues.getDueAmount())
				.minYear(membership.getCreatedAt().getYear())
				.minMonth(membership.getCreatedAt().getMonthValue())
				.build();
	}
