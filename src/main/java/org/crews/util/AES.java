...
public class AES {
    ...
    //키
    private static String AESKey; //32byte

    // 초기화 벡터
    private static String AESIv; //16byte

    @Value("${aes.algorithms}")
    public void setAlgorithms(String algorithms) {
        AES.algorithms = algorithms;
    }
    ...
    // AES 암호화
    public static String encrypt_AES(String ID){
        try {
    ...
    // AES 복호화
    public static String decrypt_AES(String ID){
        try {
            // 암호화/복호화 기능이 포함된 객체 생성
            Cipher cipher = Cipher.getInstance(algorithms);
